...
<div class="row">
<div class="col-md-12 center"><a href="<?php echo base_url(); ?>crud/editor/add" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-plus"></span>&nbsp;Dodaj tabelę</a></div> 
</div>
...
<tr>
<td>
<a class="btn btn-success btn-xs pull-left" href="<?php echo base_url().'crud/editor/edit/'.$row['scrudID']; ?>"><span class="glyphicon glyphicon-edit"></span>&nbsp;Edycja</a>
</td>
<td style="text-align:center;"><?php echo $row['scrudID'];?></td>
<td><?php echo $row['name'];?></td>
<td>
<a class="btn btn-danger btn-xs pull-right" href="javascript:confirmDialog('<?php echo base_url().'crud/editor/delete/'.$row['scrudID']; ?>');"><span class="glyphicon glyphicon-remove"></span>&nbsp;Usuń</a>
</td>
</tr>
<?php }; ?>    
...
<div id="columns">
    ...
    if(isset($columns['columnID']))
        for($i=0;$i<sizeof($columns['columnID']);$i++)
            $this->load->view('crud/editor_fields',array('action'=>'add_column','data'=>array('columnID'=>$columns['columnID'][$i], 'column_name'=>$columns['column_name'][$i], 'column_description'=>$columns['column_description'][$i],'column_align'=>$columns['column_align'][$i], 'column_width'=>$columns['column_width'][$i], 'column_params'=>$columns['column_params'][$i]    ) ) );
   ?>
</div>  
...
<div id="keys">
    ...
    if(isset($keys['keyID']))
        for($i=0;$i<sizeof($keys['keyID']);$i++)
            $this->load->view('crud/editor_fields',array('action'=>'add_key','data'=>array('keyID'=>$keys['keyID'][$i], 'key_name'=>$keys['key_name'][$i]) ) );
    ?>    
</div>
...
<div id="searchs">
    ...
    if(isset($searchs['searchID']))
        for($i=0;$i<sizeof($searchs['searchID']);$i++)
            $this->load->view('crud/editor_fields',array('action'=>'add_search','data'=>array('searchID'=>$searchs['searchID'][$i], 'search_name'=>$searchs['search_name'][$i], 'search_type'=>$searchs['search_type'][$i] ) ) );
    ?>        
</div>
...
<div id="fields">
    ...
    if(isset($fields['fieldID']))
        for($i=0;$i<sizeof($fields['fieldID']);$i++)
            $this->load->view('crud/editor_fields',array('action'=>'add_field','data'=>array('fieldID'=>$fields['fieldID'][$i], 'field_name'=>$fields['field_name'][$i],'field_label'=>$fields['field_label'][$i],'field_type'=>$fields['field_type'][$i],'field_size'=>$fields['field_size'][$i],'field_default'=>$fields['field_default'][$i],'field_mysql_function'=>$fields['field_mysql_function'][$i], 'field_data'=>$fields['field_data'][$i], 'field_rules'=>$fields['field_rules'][$i]  ) ) );
    ?>            
</div>
